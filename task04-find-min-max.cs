...
public class Program
{
    public static bool FindMinMax(int[][] array, out int min, out int max)
    {
        ...
        if (array == null)
            throw new ArgumentNullException("Null in argument. Please, enter valid argument");
        ...
        {
            if (array[k] == null || array[k].Length == 0)
                continue;
            ...
            if (array[k][array[k].Length - 1] > max)
            {
                max = array[k][array[k].Length - 1];
            }
        }
        if (!f)
        {
